#!/bin/bash
#SBATCH --job-name=kf_stage1_soap_2gpu
#SBATCH --output=logs/kf_stage1_soap_2gpu_%j.out
#SBATCH --error=logs/kf_stage1_soap_2gpu_%j.err
...
# Kolmogorov Stage A (SOAP) 雙 GPU 訓練腳本
# 維護原則：只在本地 repo 修改，遠端機器透過 git sync 後使用。

PROJECT_DIR="${PROJECT_DIR:-${HOME}/jaxpi}"
CONFIG_PATH="${CONFIG_PATH:-examples/kolmogorov_flow/stage_ab/pirate_les_stage1_soap.py}"
RUN_NAME="${RUN_NAME:-kf_stage1_soap_2gpu_${SLURM_JOB_ID}}"
...
EVAL_CONFIG_ALIAS="${EVAL_CONFIG_ALIAS:-stage1_soap}"
RUN_EVAL="${RUN_EVAL:-0}"
...
cat <<EOF
===========================
Kolmogorov Stage A SOAP 雙 GPU 訓練
===========================
Job ID: ${SLURM_JOB_ID}
Node: ${SLURM_NODELIST}
Project: ${PROJECT_DIR}
...
echo "stdout: ${PROJECT_DIR}/logs/kf_stage1_soap_2gpu_${SLURM_JOB_ID}.out"
echo "stderr: ${PROJECT_DIR}/logs/kf_stage1_soap_2gpu_${SLURM_JOB_ID}.err"
