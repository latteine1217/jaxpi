#!/bin/bash
#SBATCH --job-name=kf_paper_repro_soap
#SBATCH --output=logs/kf_paper_repro_soap_%j.out
#SBATCH --error=logs/kf_paper_repro_soap_%j.err
...
# Kolmogorov pure-PINN paper reproduction (SOAP) 雙 GPU 訓練腳本
# 維護原則：只在本地 repo 修改，遠端透過 git sync 後使用。

PROJECT_DIR="${PROJECT_DIR:-${HOME}/jaxpi}"
CONFIG_PATH="${CONFIG_PATH:-examples/kolmogorov_flow/configs/paper_repro_soap.py}"
RUN_NAME="${RUN_NAME:-kf_paper_repro_soap_${SLURM_JOB_ID}}"
...
cat <<EOF
===========================
Kolmogorov paper reproduction SOAP 雙 GPU 訓練
===========================
Job ID: ${SLURM_JOB_ID}
Node: ${SLURM_NODELIST}
Project: ${PROJECT_DIR}
...
echo "stdout: ${PROJECT_DIR}/logs/kf_paper_repro_soap_${SLURM_JOB_ID}.out"
echo "stderr: ${PROJECT_DIR}/logs/kf_paper_repro_soap_${SLURM_JOB_ID}.err"
