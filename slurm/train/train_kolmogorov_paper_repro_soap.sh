#!/bin/bash
#SBATCH --job-name=train_kf_paper_repro_soap
#SBATCH --output=logs/train_kf_paper_repro_soap_%j.out
#SBATCH --error=logs/train_kf_paper_repro_soap_%j.err
...
RUN_SLUG="${RUN_SLUG:-train_kf_paper_repro_soap}"
...
slurm_print_header \
  "Kolmogorov Paper Repro SOAP Train" \
  "Project" "${PROJECT_DIR}" \
  "Config" "${CONFIG_PATH}" \
  "Workdir" "${WORKDIR}" \
  "Wandb" "${WANDB_MODE}" \
