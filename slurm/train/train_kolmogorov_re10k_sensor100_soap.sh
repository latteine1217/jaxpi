#!/bin/bash
#SBATCH --job-name=train_kf_re10k_sensor100_soap
#SBATCH --output=logs/train_kf_re10k_sensor100_soap_%j.out
#SBATCH --error=logs/train_kf_re10k_sensor100_soap_%j.err
...
CONFIG_PATH="${CONFIG_PATH:-examples/kolmogorov_flow/configs/re10k_soap_sensor100.py}"
RUN_SLUG="${RUN_SLUG:-train_kf_re10k_sensor100_soap}"
...
slurm_print_header \
  "Kolmogorov Re=1e4 SOAP + Sensor100 Train" \
  "Project" "${PROJECT_DIR}" \
  "Config" "${CONFIG_PATH}" \
  "Workdir" "${WORKDIR}" \
  "Wandb" "${WANDB_MODE}" \
