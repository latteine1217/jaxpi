#!/bin/bash
#SBATCH --job-name=train_kf_re1e6_sensor100_w25_soap
#SBATCH --output=logs/train_kf_re1e6_sensor100_w25_soap_%j.out
#SBATCH --error=logs/train_kf_re1e6_sensor100_w25_soap_%j.err
...
RUN_SLUG="${RUN_SLUG:-train_kf_re1e6_sensor100_w25_soap}"
...
slurm_print_header \
  "Kolmogorov Re=1e6 SOAP + Sensor100 W25 Train" \
  "Project" "${PROJECT_DIR}" \
  "Config" "${CONFIG_PATH}" \
  "Workdir" "${WORKDIR}" \
  "Wandb" "${WANDB_MODE}" \
