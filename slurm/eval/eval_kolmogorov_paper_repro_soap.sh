#!/bin/bash
#SBATCH --job-name=eval_kf_paper_repro_soap
#SBATCH --output=logs/eval_kf_paper_repro_soap_%j.out
#SBATCH --error=logs/eval_kf_paper_repro_soap_%j.err
...
EVAL_DRIVER="${EVAL_DRIVER:-eval_paper_repro_soap.py}"
EVAL_CKPT_ROOT="${EVAL_CKPT_ROOT:-${PROJECT_DIR}/2026-03-17_train_re1e6_n2048_ke024_soap/ckpt}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-${PROJECT_DIR}/2026-03-16_eval_re1e6_n2048_paper_soap}"
...
slurm_print_header \
  "Kolmogorov Paper Repro SOAP Eval" \
  "Project" "${PROJECT_DIR}" \
  "Driver" "${EVAL_DRIVER}" \
  "Ckpt" "${EVAL_CKPT_ROOT}" \
