#!/bin/bash
#SBATCH --job-name=eval_kf_re10k_n256_soap
#SBATCH --output=logs/eval_kf_re10k_n256_soap_%j.out
#SBATCH --error=logs/eval_kf_re10k_n256_soap_%j.err
...
EVAL_DRIVER="${EVAL_DRIVER:-eval_re10k_n256_soap.py}"
EVAL_CKPT_ROOT="${EVAL_CKPT_ROOT:-${PROJECT_DIR}/2026-03-23_train_re10k_n256_soap/ckpt}"
EVAL_OUTPUT_DIR="${EVAL_OUTPUT_DIR:-${PROJECT_DIR}/2026-03-28_eval_re10k_n256_soap}"
...
slurm_print_header \
  "Kolmogorov Re=1e4 N256 SOAP Eval" \
  "Project" "${PROJECT_DIR}" \
  "Driver" "${EVAL_DRIVER}" \
  "Ckpt" "${EVAL_CKPT_ROOT}" \
