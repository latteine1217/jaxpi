...
def get_config():
    """Window-1 sensor100 rerun that keeps all checkpoints every 10000 steps."""
    config = base_config.get_config()

    config.wandb.name = "re1e6_n512_ds4_soap_sensor100_w50_w1_ckpt50k100k"
    config.wandb.group = "re1e6_window1_ckpt_validation"
    config.wandb.tags = list(config.wandb.tags) + ["save_every_10000", "keep_all_ckpts"]
    config.wandb.notes = (
        "Sensor100 window-1 rerun for checkpoint validation. "
