...
def get_config():
    """SOAP + 100 QR-pivot sensors, 50 windows, aligned to current N512 ds4 run."""
    ...
    wandb.notes = (
        "SOAP + 100 QR-pivot sensors on the current Re=1e6, N=512, T=5, ds4 DNS. "
        "A/B run against the failed w25 sensor setup: keep sensor constraints and optimizer fixed, "
        "but restore 50 windows (0.1s/window) to test whether the shorter horizon prevents cross-window drift."
    )
    wandb.sweep_id = None
...
def _validate_config(config):
    ...
    if config.use_vorticity_data_loss:
        raise ValueError("paper_repro_soap_sensor100_n512_w50 禁止啟用 w_data loss")

    print(
        f"✓ Sensor-100 N512 w50 config validated: batch_size_per_device={batch_size_per_device}, "
