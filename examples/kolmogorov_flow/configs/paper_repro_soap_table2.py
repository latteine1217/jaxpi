...
def get_config():
    """Table 2-oriented pure PINN reproduction config for Kolmogorov flow (SOAP)."""
    config = ml_collections.ConfigDict()

    config.mode = "train"

    # 對齊 2507.08972v2 的 Table 2：time marching + 跨 window transfer learning。
    ...
    wandb.name = "paper_repro_soap_table2"
    ...
    wandb.notes = (
        "2507.08972v2 Table 2-oriented pure PINN Kolmogorov reproduction: PirateNet, "
        "2 residual blocks, hidden_dim=768, swish, SOAP, schedule_free=True, "
        "warmup=2000, no data constraints."
    )
    wandb.sweep_id = None
    ...
    # Table 2: two residual blocks, hidden size 768, Swish.
    arch.num_layers = 2
    arch.hidden_dim = 768
    arch.out_dim = 3
    ...
    # Table 2 / appendix: Fourier features sampled from N(0, 2).
...
def _validate_config(config):
    ...
    print(
        f"✓ Paper Table 2 repro config validated: batch_size_per_device={batch_size_per_device}, "
