...
def get_config():
    """Strict Table 2-oriented pure PINN reproduction config for Kolmogorov flow (SOAP)."""
    config = ml_collections.ConfigDict()

    config.mode = "train"

    # Table 2 only lists the time-marching setup at a high level; keep minimal runnable assumptions.
    ...
    wandb.name = "paper_repro_soap_table2_strict"
    ...
    wandb.notes = (
        "2507.08972v2 strict Table 2-oriented pure PINN Kolmogorov reproduction: PirateNet, "
        "2 residual blocks, hidden_dim=768, swish, SOAP, warmup=2000, no schedule_free, "
        "no grad clipping, no data constraints."
    )
    wandb.sweep_id = None
    ...
    # Table 2 batch size = 8192；雙卡時對應 4096 per device。
    ...
    # Keep engineering defaults minimal and neutral.
...
def _validate_config(config):
    ...
    print(
        f"✓ Strict Table 2 repro config validated: batch_size_per_device={batch_size_per_device}, "
